...
const CandidateDashboard = () => {
  ...
  const calculateUserScores = () => {
    ...
    assessmentQuestions.forEach((question) => {
      ...
      const tagName = question.tag.tagname; // Adjust this line based on your actual data structure
      calculatedTagNames.set(tagId, tagName);
    });

    setTagNames(calculatedTagNames);
    console.log("Calculated Tag Names", calculatedTagNames);
    // Assuming you have state variables candidateResults and assessmentQuestions
    // Map questions to tags
    const questionTagMap = new Map();
    assessmentQuestions.forEach((question) => {
      const tagId = question.tag.tagid; // Adjust this line based on your actual data structure
    ...
    candidateResults.forEach((result) => {
      const tagId = result.question.tag.tagid; // Adjust this line based on your actual data structure
  ...
  const handleClick = () => {
    ...
    if (candidateResults.length > 0 && assessmentQuestions.length > 0) {
      calculateUserScores();
    }
    else {
      // No assessment results available, show alert
      alert("No Progress results available.");
    }
  };
            ...
            {/* <div className="status">
              ...
              <div className="course-completion">
                ...
                {courseStatus.some((status) => status !== "") ? (
                  ...
                  <p>No course completion information available.</p>
                )}
              </div>
            </div> */}
