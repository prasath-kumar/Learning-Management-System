  ...
  const toggleManageFeedback = async () => {
    ...
    try {
      const responseadmin = await axios.get(
        `${API_BASE_URL}/feedback/user/${userDetails.userid}`
      );

      const responsecandidate = await axios.get(
        `${API_BASE_URL}/feedback/user/usertype/${usertypeadmin}`
      );
      const filteredFeedbacks = responsecandidate.data.filter(
        (feedback) => feedback.recipientUser.userid === userDetails.userid
      );

      setFeedbacksSent(responseadmin.data);
      setFeedbacksReceived(filteredFeedbacks);
    } catch (error) {
            ...
            <div className="sent-feedbacks">
              ...
              {feedbacksSent.map((feedback) => (
                <div key={feedback.id} className="feedback-item">
                  <p>Candidate ID: {feedback.user?.userid}</p>
                  <p>Candidate Name: {feedback.user?.username}</p>
